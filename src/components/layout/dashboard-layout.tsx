  ...
  return (
    <SidebarProvider>
      <Sidebar className="border-r border-slate-200 dark:border-slate-800">
        <SidebarHeader className="h-16 flex items-center px-6 border-b">
          <Link href="/dashboard" className="flex items-center gap-2">
            ...
            <span className="font-headline font-bold text-xl tracking-tight text-primary">StudyQuest</span>
          </Link>
        </SidebarHeader>
        ...
        <SidebarFooter className="p-4 border-t">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild className="h-11 px-4 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
      ...
      <SidebarInset className="bg-slate-50 dark:bg-slate-950/50">
        <header className="h-16 flex items-center justify-between px-4 md:px-8 bg-white dark:bg-slate-900 border-b sticky top-0 z-20 transition-all">
          <div className="flex items-center gap-2 md:gap-4">
            <SidebarTrigger className="h-9 w-9" />
            <div className="h-4 w-px bg-slate-200 dark:bg-slate-800 hidden md:block" />
            <h2 className="font-bold text-sm md:text-lg text-slate-900 dark:text-white truncate max-w-[150px] md:max-w-none">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="relative rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 h-10 w-10 transition-colors"
                  onClick={() => setHasNewNotifications(false)}
                >
                  <Bell className="h-5 w-5 text-slate-600 dark:text-slate-400" />
                  {hasNewNotifications && (
                    <span className="absolute top-2.5 right-2.5 h-2.5 w-2.5 bg-red-500 rounded-full border-2 border-white dark:border-slate-900 animate-pulse" />
                  )}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-80 p-0 rounded-2xl shadow-2xl border-none" align="end">
                ...
                <ScrollArea className="h-[350px]">
                  <div className="grid">
                    {mockNotifications.map((notif) => (
                      <button 
                        key={notif.id} 
                        className="flex items-start gap-3 p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b last:border-0 border-slate-100 dark:border-slate-800"
                      >
                        <div className={cn("mt-1 p-2 rounded-lg bg-slate-100 dark:bg-slate-800", notif.color)}>
                ...
                <div className="p-2 border-t dark:border-slate-800 text-center">
                  <Button variant="ghost" className="text-xs w-full h-9 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">View all quest activity</Button>
                </div>
              </PopoverContent>
            </Popover>
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 h-10 w-10 hidden md:flex">
